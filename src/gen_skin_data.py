...
def run(api_data) -> dict[str, Skin]:
    ...
    for datum in api_data:
        ...
        available_conditions = get_all_conditions_for_float_range(
            min_float, max_float)
        for condition in available_conditions:
            ...
            full_unformatted_name = remove_skin_name_formatting(
                full_formatted_name)
            description_match = re.search(r"<i>(.*?)</i>",
                                          datum["description"])
...
if __name__ == "__main__":
    ...
    with open(f"{OUTPUT_DIRECTORY}/skin_data.json", "w+",
              encoding="utf-8") as f:
